...
SELECT title, description, rating, length FROM film_list WHERE length >= 180;
...
SELECT payment_id, amount, payment_date FROM payment WHERE payment_date >= 20050527;
...
SELECT
    ...
    payment_date,
    amount
FROM payment WHERE amount > 5 LIMIT 1000, 1000;
...
SELECT
    ...
    rental_duration
FROM film WHERE 
    special_features = 'Behind the Scenes' AND
    length < 120 AND 
...
# left join between the customer and actor column on the last_name column in each table. (i.e. `customer.last_name = actor.last_name`)
...
# 9b. Select the customer first_name/last_name and actor first_name/last_name columns from 
# performing a right join between the customer and actor column on the last_name column in each table. 
...
# 9c. Select the customer first_name/last_name and actor first_name/last_name columns from 
# performing an inner join between the customer and actor column on the last_name column in each table. 
# (i.e. `customer.last_name = actor.last_name`)
SELECT
    customer.first_name AS customer_first_name, 
    customer.last_name AS customer_last_name,
...
# 9e. Select the title, description, release year, and language name columns
# from the film table, performing a left join with the language table to get the "language" column.
...
# from the staff table, performing 2 left joins with the address table then the city table to get the address and city related columns.
SELECT
    first_name,
    last_name,
    address,
